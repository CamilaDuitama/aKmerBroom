...
def getbloomFilter(bf, bf_capacity, kmers_file: Union[str, bytes, os.PathLike], kmer_size: int, output: Union[str, bytes, os.PathLike]):
    ...
    if os.path.isfile(bf):
        logger.info("Opening Bloom Filter of modern or ancient kmers")
        kmers_bf = BloomFilter.open(bf)
        logger.info("Done")
    else:
        logger.info("Need to make Bloom Filter of modern or ancient k-mers")
...
def classify_reads(fastq_file: Union[str, bytes, os.PathLike], bloom_filter, kmer_size: int, n_consecutive_matches: int, output: Union[str, bytes, os.PathLike], shared_anchor_kmer_set):
    ...
    for record in SeqIO.parse(fastq_file, "fastq"):
        ...
        count_of_modern_kmers_in_this_read = 0
        for i in range(0, length - kmer_size + 1):
            ...
            if kmer in bloom_filter or reverse[length - kmer_size - i:length - i] in bloom_filter:
                count_of_modern_kmers_in_this_read += 1
                matches.append(0)    # match to modern kmer found, represented by '!'
            else:
                matches.append(12)   # match to modern kmer not found, represented by '-'
...
def classify_reads_using_anchor_kmers(input_file: Union[str, bytes, os.PathLike], anchor_kmer_set, kmer_size: int, anchor_proportion_cutoff: float, output: Union[str, bytes, os.PathLike], modern_DNA_flag: bool):
    """
    ...
    :param kmer_size: int
    :param anchor_proportion_cutoff: float for the proportion of kmers that must be recognised to classify the read depending on the modern flag
    :param output: string/path to output directory.
    :param modern_DNA_flag: boolean to indicate if the reference given was modern DNA or not
    :return: nothing
    """
    ...
    for record in SeqIO.parse(ip_reads_file, "fastq"):
        ...
        try:
            ...
            anchor_proportion = 0
        # select reads if they meet the proportion cutoff
        if anchor_proportion >= anchor_proportion_cutoff:
            ...
            if modern_DNA_flag :
                #if the reference is modern DNA, then the recognised reads are contamination
